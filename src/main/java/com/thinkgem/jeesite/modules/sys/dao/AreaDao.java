...
public interface AreaDao extends AreaDaoCustom, CrudRepository<Area, Long> {
	...
	@Modifying
	@Query("update Area set delFlag='" + Area.DEL_FLAG_DELETE
	...
	@Query("from Area where delFlag='" + Area.DEL_FLAG_NORMAL
			+ "' order by code")
	public List<Area> findAllList();

	@Query("from Area where (id=?1 or parent.id=?1 or parentIds like ?2) and delFlag='"
			+ Area.DEL_FLAG_NORMAL + "' order by code")
