...
public interface RoleDao extends RoleDaoCustom, CrudRepository<Role, Long> {
	...
	@Modifying
	@Query("update Role set delFlag='" + Role.DEL_FLAG_DELETE
			+ "' where id = ?1")
	...
	@Query("select r from Role r, User u where r in elements (u.roleList) and r.delFlag='"
			+ Role.DEL_FLAG_NORMAL
			+ "' and u.delFlag='"
			+ User.DEL_FLAG_NORMAL
			+ "' and u.id=?1 order by r.name")
...
class RoleDaoImpl extends BaseDaoImpl<Role> implements RoleDaoCustom {
	...
	// private static final String QUERY_USER_BY_GROUPID =
	// "select u from User u left join u.roleList g where g.id=?";
	//
	// @Override
	...
	// List<User> users =
	// getEntityManager().createQuery(QUERY_USER_BY_GROUPID).setParameter(1,
	// id).getResultList();
	// for (User u : users) {
