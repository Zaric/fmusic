...
public class CmsUtils implements ApplicationContextAware {
	 ...
	 */
	public static List<Category> getCategoryList(long siteId, long parentId,
	 ...
	 *            推荐位（1：首页焦点图；2：栏目页文章推荐；） thumb 缩略图（1：有缩略图的文章）
	 * @return
	 */
	public static List<Article> getArticleList(long siteId, long categoryId,
			int number, String param) {
		Page<Article> page = new Page<Article>(1, number, -1);
		Article article = new Article(
				new Category(categoryId, new Site(siteId)));
		if (StringUtils.isNotBlank(param)) {
	 ...
	 * @return
	 */
	public static List<Link> getLinkList(long siteId, long categoryId,
			int number, String param) {
	...
	public void setApplicationContext(ApplicationContext applicationContext) {
		...
		articleService = (ArticleService) applicationContext
