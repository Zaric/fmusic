...
public class ArticleService extends BaseService {
	...
	public Page<Article> find(Page<Article> page, Article article) {
		...
		if (StringUtils.isNotEmpty(article.getThumb())
				&& "1".equals(article.getThumb())) {
			dc.add(Restrictions.and(Restrictions.isNotNull("thumb"),
					Restrictions.ne("thumb", "")));
		}
	...
	public void delete(Long id, Boolean isRe) {
		...
		// isRe!=null&&isRe?Article.STATUS_RELEASE:Article.STATUS_DELETE);
		// 使用下面方法，以便更新索引。
		Article article = articleDao.findOne(id);
		article.setStatus(isRe != null && isRe ? Article.STATUS_RELEASE
				: Article.STATUS_DELETE);
		articleDao.save(article);
	}
		...
		if (idss.length > 0) {
			...
			for (Article e : l) {
				list.add(new Object[] { e.getCategory().getId(), e.getId(),
						StringUtils.abbreviate(e.getTitle(), 20) });
			}
		}
		return list;
	}
	...
	public Page<Article> search(Page<Article> page, String q) {
		...
		BooleanQuery query = articleDao.getFullTextQuery(q, "title",
				"keywords", "desciption", "articleData.content");
		// 设置过滤条件
		BooleanQuery queryFilter = articleDao
				.getFullTextQuery(new BooleanClause(new TermQuery(new Term(
						"status", Article.STATUS_RELEASE)), Occur.MUST));
		// 设置排序
		Sort sort = new Sort(new SortField("updateDate", SortField.DOC, true));
		// 全文检索
		articleDao.search(page, query, queryFilter, sort);
		// 关键字高亮
		articleDao.keywordsHighlight(query, page.getList(), "desciption",
				"articleData.content");
