...
public class FrontController extends BaseController {
	...
	public String list(
		...
		if ("2".equals(category.getShowModes())
			...
			List<Category> categoryList = categoryService.findByParentId(
					article.getCategory().getParent().getId(), category
							.getSite().getId());
			model.addAttribute("category", article.getCategory());
			model.addAttribute("categoryList", categoryList);
			model.addAttribute("article", article);
			return "modules/cms/front/themes/" + category.getSite().getTheme()
					+ "/frontViewArticle";
		} else {
			List<Category> categoryList = categoryService.findByParentId(
			...
			if ("1".equals(category.getShowModes()) || categoryList.size() == 0) {
				...
				return "modules/cms/front/themes/"
						+ category.getSite().getTheme() + "/frontList";
			}
			// 有子栏目：显示子栏目列表
			else {
					...
					if (Category.SHOW.equals(c.getInList())) {
						if ("article".equals(c.getModule())) {
							categoryMap.put(
									c,
									articleService.find(
											new Page<Article>(1, 5, -1),
											new Article(c)).getList());
						} else if ("link".equals(c.getModule())) {
							categoryMap.put(
									c,
									linkService.find(new Page<Link>(1, 5, -1),
											new Link(c)).getList());
						}
					}
				}
				...
				return "modules/cms/front/themes/"
						+ category.getSite().getTheme() + "/frontListCategory";
			}
		}
	}
	...
	public String view(@PathVariable Long categoryId,
			@PathVariable Long contentId, Model model) {
		Category category = categoryService.get(categoryId);
		if (category == null) {
			Site site = CmsUtils.getSite(1L);
			model.addAttribute("site", site);
		...
		if ("article".equals(category.getModule())) {
			...
			if (article == null
					|| !Article.STATUS_RELEASE.equals(article.getStatus())) {
				return "error/404";
			}
			model.addAttribute("article", article);
			model.addAttribute("category", article.getCategory());
			...
			model.addAttribute("categoryList", categoryList);
			// 获取推荐文章列表
			List<Object[]> relationList = articleService.findByIds(article
					.getArticleData().getRelation());
	...
	public String commentSave(Comment comment, String validateCode,
			@RequestParam(required = false) Long replyId, Model model,
