...
public class BaseDaoImpl<T> implements BaseDao<T> {
	...
	public Page<T> find(Page<T> page, String qlString, Object... parameter) {
		...
		if (!page.isDisabled() && !page.isNotCount()) {
			...
			page.setCount((Long) createQuery(countQlString, parameter)
					.getSingleResult());
			if (page.getCount() < 1) {
				return page;
			}
	...
	public Page<T> findBySql(Page<T> page, String qlString, Object... parameter) {
		...
		if (!page.isDisabled() && !page.isNotCount()) {
			String countQlString = "select count(*)"
					+ removeSelect(removeOrders(qlString));
			page.setCount((Long) createSqlQuery(countQlString, parameter)
					.getSingleResult());
			if (page.getCount() < 1) {
				return page;
			}
	...
	public long count(DetachedCriteria detachedCriteria) {
		long count = ((Number) detachedCriteria
		...
		for (T entity : list) {
			try {
				for (String field : fields) {
					...
					String desciption = highlighter.getBestFragment(analyzer,
							field, text);
					if (desciption != null) {
